import type { Vitals } from '@raga/shared-types'
...
function status(val: number, low: number, high: number) {
...
export default function VitalsPanel({ vitals }: Props) {
  const rows: VitalRow[] = [
    {
      label: 'Heart Rate',
  ...
  return (
    ...
    >
      <div className="px-5 py-4 border-b border-slate-50">
        ...
        <p className="text-xs text-slate-400 mt-0.5">Latest recorded values</p>
      </div>
      ...
      <div className="divide-y divide-slate-50">
        {rows.map(row => {
          ...
          const pct = Math.min(
            100,
            ((row.raw - row.low * 0.6) / (row.high * 1.2 - row.low * 0.6)) * 100
          )
          ...
          return (
            <div key={row.label} className="flex items-center gap-4 px-5 py-3.5">
              ...
              >
                <span className={cfg.text}>{row.icon}</span>
              </div>
              ...
              <div className="text-right shrink-0">
                <p className="text-sm font-bold text-slate-800 tabular-nums">{row.value}</p>
...
const TempIcon = () => (
  <svg viewBox="0 0 20 20" fill="currentColor" className={ic}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M10 2a1 1 0 011 1v7.586l2.707 2.707A1 1 0 0112 15H8a1 1 0 01-.707-1.707L10 10.586V3a1 1 0 011-1z"
    />
  </svg>
)
const LungIcon = () => (
...
const WeightIcon = () => (
  <svg viewBox="0 0 20 20" fill="currentColor" className={ic}>
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M10 2a4 4 0 00-4 4H5a2 2 0 00-2 2v8a2 2 0 002 2h10a2 2 0 002-2V8a2 2 0 00-2-2h-1a4 4 0 00-4-4zm0 2a2 2 0 100 4 2 2 0 000-4zm0 9a3 3 0 100-6 3 3 0 000 6z"
    />
  </svg>
)
