...
export function Badge({ status, variant = 'subtle', className }) {
    const config = statusMap[status] ?? {
        label: status,
        solid: 'bg-slate-500 text-white',
        subtle: 'bg-slate-100 text-slate-600 ring-1 ring-slate-200',
    };
    return (_jsx("span", { className: clsx('inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium capitalize', variant === 'solid' ? config.solid : config.subtle, className), children: config.label }));
}
